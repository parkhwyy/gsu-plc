...
class Rexp {
	...
	Rexp() {
		...
		if ((Lexer.nextToken == Token.GREATER_OP) || (Lexer.nextToken == Token.GREATER_EQ_OP) || (Lexer.nextToken == Token.LESSER_OP)
		|| (Lexer.nextToken == Token.LESSER_EQ_OP) || (Lexer.nextToken == Token.EQ_OP) ||(Lexer.nextToken == Token.NOT_EQ)) {
			Lexer.lex();
			e1 = new Expr();
			c.genbyte_rexp(op);
		}
